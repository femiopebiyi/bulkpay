...
export default function Navbar({ activePage, onNavigate }: NavbarProps) {
  ...
  return (
    <nav className="bg-bp-dark">
      <div className="max-w-[1060px] mx-auto px-4 sm:px-6 flex items-center h-12 gap-1">
        {/* Logo */}
        <div
          className="flex items-center gap-1.5 mr-3 sm:mr-4 cursor-pointer flex-shrink-0"
        ...
        <div className="flex items-center gap-0.5 overflow-x-auto no-scrollbar flex-1 min-w-0">
          {NAV_ITEMS.map((item) => (
            <button
              ...
              className={`text-xs px-2 sm:px-2.5 py-1 rounded whitespace-nowrap
        ...
        <div className="relative flex-shrink-0" ref={popupRef}>
          <button
            ...
            className="font-mono text-[11px] text-bp-hint bg-white/[0.06]
                       px-2.5 py-1 rounded border border-white/10
          ...
          {showPopup && (
            <div className="absolute right-0 top-full mt-1.5 w-40 bg-white
                            border border-bp-border rounded-lg shadow-sm z-50 animate-fade-in overflow-hidden">
              <button
                onClick={() => {
                  setShowPopup(false);
                  onNavigate("profile");
