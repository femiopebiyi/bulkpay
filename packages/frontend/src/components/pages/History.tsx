...
export default function History({ onOpenBatch }: Props) {
  return (
        ...
        {mockBatches.map((b) => (
          <div
            ...
            className="flex flex-col sm:flex-row sm:items-center px-3 py-2.5 border-b border-gray-50
                       hover:bg-gray-50 transition-colors cursor-pointer last:border-b-0 gap-1 sm:gap-0"
          >
            ...
            <div className="sm:w-[17%]">
              <span className={`inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded font-medium ${badge[b.status]}`}>
                <span className="w-1 h-1 rounded-full bg-current" />
                {label[b.status]}
              </span>
            </div>
            <div className="sm:w-[20%] font-mono text-[10px] text-bp-hint">
              {b.txSignature || "—"}
            </div>
          </div>
        ))}
