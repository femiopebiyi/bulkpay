...
export default function Dashboard({ onNavigate, onOpenBatch, onNewSchedule }: {
  onNavigate: (p: Page) => void; onOpenBatch: (b: BatchRecord) => void; onNewSchedule: () => void;
}) {
  ...
  return (
    <div className="animate-slide-up">
      <h1 className="font-display text-xl sm:text-[22px] text-bp-dark tracking-tight">{greeting}, {profile.name}</h1>
        ...
        {[
          ...
          <div key={s.label} className="bg-bp-dark rounded-lg p-3 sm:p-3.5">
            ...
            <div className="font-mono text-lg sm:text-xl font-medium text-white">
              {s.value} <span className="text-bp-accent text-[11px]">{s.unit}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={() => onNavigate("send")} className="bg-bp-accent text-bp-dark font-medium text-xs px-4 py-2 rounded-md hover:bg-bp-accent-hover transition-all active:scale-[0.98] cursor-pointer">+ New batch</button>
        <button onClick={onNewSchedule} className="bg-bp-dark-btn text-white font-medium text-xs px-4 py-2 rounded-md hover:bg-bp-dark-btn-hover transition-all cursor-pointer">New schedule</button>
      </div>
      ...
      <div className="bg-white border border-bp-border rounded-lg overflow-hidden">
        <div className="flex items-center px-3 py-2 border-b border-bp-border-light text-[11px] text-bp-hint tracking-wide uppercase">
          <div className="w-[45%] sm:w-[40%]">Batch</div><div className="w-[15%] text-center">Recip.</div>
          <div className="w-[20%]">Total</div><div className="w-[20%] sm:w-[25%]">Status</div>
        </div>
        {mockBatches.map((b) => (
