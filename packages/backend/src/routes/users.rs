...
pub async fn get_user(
    ...
    .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, "Invalid program ID".to_string()))?;

    let (user_account_pda, _) = Pubkey::find_program_address(
        &[b"useraccount", pubkey.as_ref()],
        &program_id,
    );

    // Read all_time_amount_sent from on-chain — offset 8 (discriminator) + 32 (owner) = 40
    // UserAccount layout: discriminator(8) + owner(32) + all_time_amount_sent(8) + bump(1) + is_created(1)
    let all_time_sent = match state.rpc.get_account_data(&user_account_pda) {
        Ok(data) if data.len() >= 48 => {
            i64::from_le_bytes(data[40..48].try_into().unwrap_or([0u8; 8]))
        }
        _ => 0, // account doesn't exist yet — new user
    };
